	...
	dashboardPage(
		...
		dashboardBody(
			tabItems(
				...
				#panel crear usuario, solo admin
				tabItem(
					tabName="create_users",
					fluidRow(
					  box(title = "Crear nuevo usuario", width = 6, status="primary",
					    textInput("new_user", "Nombre de usuario"),
					    passwordInput("new_pass", "Contraseña temporal"),
					    selectInput("new_role", "role", choices=c("admin","viewer")),
					    actionButton("create_button", "Crear usuario"),
					    verbatimTextOutput("creation_msg")
					  )
					)
				)
			)
...
server <- function(input, output, session){
	...
	options("shinymanager.pwd_validity" = 60)
	add_user_secure <- function(username, password){
		cred <- shinymanager::read_db_decrypted(
			db = ruta_usuarios_sqlite, passphrase = key_get("R-shinymanager-key", "nicol")
		)
		
		if(username %in% cred$user){
			return ("El usuario ya existe.")
		}
		nuevo_usuario <- data.frame(
			user = username, password = password, admin=FALSE, must_change_password=1, stringsAsFactors = FALSE
		)

		cred_actualizado <- rbind(cred, nuevo_usuario)

		create_db(
			credentials_data = cred_actualizado,
			sqlite_path = ruta_usuarios_sqlite,
			passphrase = key_get("R-shinymanager-key", "nicol")

		)
		return("Usuario creado correctamente. Deberá cambiar la contraseña al iniciar sesión por primera vez.")
	}
	...
	observeEvent(input$create_button, {
		...
		if (user=="" || pw==""){
			output$creation_msg <- renderText("Usuario y contraseña no pueden estar vacíos.")
			return()
		}

		if (!validate_password_custom(pw)){
			output$creation_msg <- renderText("La contraseña debe tener al menos 8 caracteres, incluyendo mayúsculas, minúsculas, números y caracteres especiales")
			return()
		}

		#pw_hash <- scrypt::hashPassword(pw)

		#dbExecute(db_conn, "INSERT INTO credentials (user, password, admin, must_change_password) VALUES (?, ?, ?, 1)",
		#	params = list(user, pw_hash, FALSE)
		
		#)

		msg <- add_user_secure(username = user, password = pw)
		
		output$creation_msg <- renderText(msg)
	})
...
#  		}
	...
	validate_password_custom <- function(pw){
		nchar(pw) >= 8 && grepl("[A-Z]",pw) && grepl("[a-z]", pw) && grepl("[0-9]", pw) && grepl("[[:punct:]]", pw)
	}
	...
	secure_credentials <- function(users_db_path, passphrase){
		function(user, password, session){
			...
			if(result$result){
				...
				#	db =  ruta_usuarios_sqlite, passphrase = key_get("R-shinymanager-key", "nicol")					
				#)
				#user_data <- cred[cred$user == user_logged, ]
				#if(nrow(user_data) > 0 && user_data$must_change_password == 1){
				#	showModal(modalDialog(
				#		title = "Cambio de contraseña obligatorio",
				#		passwordInput("new_pass", "Nueva contraseña"),
				#		passwordInput("confirm_pass", "Confirmar nueva contraseña"),
				#		footer = tagList(actionButton("guardar_nueva", "Guardar"), modalButton("Cancelar"))
				#	))
				#	return(NULL)
				#}
...
#	observeEvent(input$guardar_nueva, {
#		new_pw <- input$new_pass
#		conf_pw <- input$confirm_pass
#		new_usr <- isolate(input$new_user)
#
#		if (new_pw != conf_pw){
#			showNotification("Las contraseñas no coinciden.", type="error")
#			return()
#		}
#		if (!validate_pass(new_pw)){
#			showNotification("La contraseña debe tener al menos 8 caracteres, incluyendo mayúsculas, minúsculas, números y caracteres especiales.", type="error")
#			return()
#		}
#		hashed_pw <- scrypt::hashPassword(new_pw)
#
#		dbExecute(db_conn, "UPDATE credentials SET password = ?, must_change_password = 0 WHERE user =?", 
#		params= list(hashed_pw, new_usr))
#
#		removeModal()
#		showNotification("Contraseña actualizada correctamente. Inicie sesión con las nuevas credenciales.", type="message")
#
#	})
	...
	output$sidebar <- renderMenu({
		if (user_role()) {
  		sidebarMenu(
			...
			menuItem("Crear usuario", tabName="create_users", icon=icon("user-plus"))
		)
		}else{
		sidebarMenu(
			menuItem("Panel principal", tabName="dashboard", icon=icon("tachometer-alt"))
